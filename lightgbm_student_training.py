...
from .lightgbm_student_model import LightGBMStudentDistiller, schema_hash
...
def _acceptance_test(onnx_path: str, X_val: np.ndarray, p_val_framework: np.ndarray, tol: float = 1e-5) -> dict:
    if ort is None:
...
def main():
    ...
    ap.add_argument("--features_npz", required=True, help=".npz with X_train, X_val, feature_names (list)" )
