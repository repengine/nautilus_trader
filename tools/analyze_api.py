...
def analyze_api_index(index_path: Path):
    ...
    for item in data:
        if 'test' in item['module']:
            base = item['module'].split('.test_')[0] if '.test_' in item['module'] else item['module']
    ...
    undocumented_public = []
    for item in data:
        if (item['kind'] in ['class', 'function'] and 
            not item.get('docstring') and 
            'test' not in item['module'] and
