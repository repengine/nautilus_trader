        ...
        try:
            ...
            if len(df) > 0:
                ...
                if start_date > target_start:
                    ...
                    results["gaps"][
                        "ohlcv_daily_historical"
                    ] = f"Missing {gap_days} days of historical data ({target_start.date()} to {start_date.date()})"

                if end_date < target_end:
                    gap_days = (target_end - end_date).days
                    results["gaps"][
                        "ohlcv_daily_recent"
                    ] = f"Missing {gap_days} days of recent data ({end_date.date()} to {target_end.date()})"
        ...
        try:
            ...
            if len(df) > 0:
                ...
                if start_date > target_start:
                    ...
                    results["gaps"][
                        "ohlcv_1m_historical"
                    ] = f"Missing {gap_days} days of historical 1m data ({target_start.date()} to {start_date.date()})"

                if end_date < target_end:
                    gap_days = (target_end - end_date).days
                    results["gaps"][
                        "ohlcv_1m_recent"
                    ] = f"Missing {gap_days} days of recent 1m data ({end_date.date()} to {target_end.date()})"
            ...
            try:
                ...
                if "ts_event" in df.columns:
                    ...
                    if start_date > target_start:
                        ...
                        results["gaps"][
                            "l1_bbo_historical"
                        ] = f"Missing {gap_days} days of historical L1 BBO ({target_start.date()} to {start_date.date()})"
