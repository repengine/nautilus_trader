...
class SimpleControlPanel:
    ...
    def _maybe_get_integration(self) -> MLIntegrationManager | None:
        ...
        if self._integration_failed:
            return None
        try:
            self._integration = MLIntegrationManager(
    ...
    def _collect_store_health(self) -> dict[str, dict[str, bool]]:
        ...
        for name, store in mapping.items():
            if store is None:
                ...
                continue
            health = getattr(store, "health_check", None)
            if callable(health):
                try:
                    report = health()
                    ...
                    pass
            result[name] = {"healthy": True, "fallback": False}
        return result
    ...
    def start_actor(self, actor_id: str, actor_type: str, config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    def trigger_pipeline(self, mode: str, config: Mapping[str, Any]) -> dict[str, Any]:
        """Record a pipeline invocation."""
        run_id = f"run_{_utc_now().strftime('%Y%m%d_%H%M%S')}"
        record = _PipelineRecord(mode=mode, config=dict(config), started_at=_utc_now())
        self._pipelines[run_id] = record
        self._save_state()
        return {
            "success": True,
            "run_id": run_id,
    ...
    def trigger_backfill(self, symbols: Sequence[str], start: datetime, end: datetime) -> dict[str, Any]:
