...
def apply_observability_indices(engine: Engine) -> None:
    ...
    Safe to run multiple times. Partitioning support can be added in a future
    migration; this function focuses on low-cost indexing improvements.
    """
...
def ensure_monthly_partitions(engine: Engine, table: str, ts_col: str) -> None:
    """
    Ensure `table` is partitioned monthly on `ts_col` and that partitions exist for
    the current and next month. If the existing table is not partitioned and is
    empty, it will be dropped and recreated as a partitioned parent to avoid data
    migration complexity. No-op on non-PostgreSQL backends.
    """
    ...
    with engine.begin() as conn:
        ...
        if exists:
            ...
            if not is_partitioned:
                ...
                if int(rowcount or 0) == 0:
                    ...
                    conn.execute(text(f"CREATE TABLE {table} ({ts_col} BIGINT NOT NULL) PARTITION BY RANGE ({ts_col})"))
                    is_partitioned = True
        else:
            # Fresh create
            conn.execute(text(f"CREATE TABLE {table} ({ts_col} BIGINT NOT NULL) PARTITION BY RANGE ({ts_col})"))
        ...
        for dt in (now, datetime(now.year + (now.month // 12), ((now.month % 12) + 1), 1, tzinfo=UTC)):
            ...
            conn.execute(text(f"CREATE INDEX IF NOT EXISTS {part_name}_{ts_col}_brin ON {part_name} USING BRIN ({ts_col})"))
