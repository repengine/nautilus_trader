...
class TFTDatasetBuilder:
    ...
    def __init__(
        ...
        *,
        ...
        include_earnings : bool, optional
            Whether to augment datasets with earnings features fetched via the
            DataStore facade. Requires ``data_store`` and Polars support.
        earnings_lag_days : int, optional
            Publication lag applied to earnings filings before features become
            visible to the builder. Must be non-negative.
    ...
    def _fetch_earnings_features(
        ...
        *,
        ...
        ----------
        ticker : str
            Instrument ticker (venue suffix optional).
        timestamps : polars.Series
            Bar timestamps used for the dataset.
        as_of_date : datetime | None, optional
            Cutoff date to enforce point-in-time correctness.

        Returns
        -------
        polars.DataFrame | None
            DataFrame with earnings features aligned to timestamps, or ``None``
            when data is unavailable.
        """
        if not self.include_earnings or self.data_store is None:
            return None
        if pl is None:
        ...
        as_of_ts = self._now_ns()
        ts_max_ns = self._coerce_to_ns(timestamps.max())
        if ts_max_ns is not None:
            as_of_ts = ts_max_ns
        if as_of_date is not None:
            as_of_ts = min(as_of_ts, self._datetime_to_ns(as_of_date, fallback=as_of_ts))

        actuals_desc = self.data_store.get_earnings_actuals_at_or_before(
            ticker=base_ticker,
            ts_event=as_of_ts,
            limit=6,
        )
        ...
        def _to_iso(value: object) -> str:
            ...
            if callable(iso_fun):
                iso_callable = cast(Callable[[], object], iso_fun)
                try:
        ...
        earnings_joined = cast(PolarsDataFrame, earnings_joined_any)
        earnings_joined = earnings_joined.drop(["period_end", "filing_date", "effective_date"], strict=False)
