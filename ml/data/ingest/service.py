...
class DatabentoIngestionService:
        ...
        for symbol in symbol_list:
            ...
            for chunk_window in windows:
                ...
                if chunk_frame.empty and self._should_use_itch_fallback(
                    ...
                    fallback_frame = self._attempt_fallback_to_itch(
                        schema=schema,
                        symbol=symbol_effective,
                ...
                if chunk_frame.empty:
                    continue
                chunk_frame = self._canonicalize_chunk(
                    dataset=dataset,
                    schema=schema,
                    frame=chunk_frame,
                    symbol=download_symbol,
    ...
    def _canonicalize_chunk(
        ...
        *,
        ...
        del schema, symbol, instrument_id  # Unused in passthrough mode
        if frame.empty:
            return frame
        working = frame.copy()
        if "source_dataset" not in working.columns:
            working["source_dataset"] = source_dataset or dataset
        return working
    ...
    def _attempt_fallback_to_itch(
        ...
        *,
        ...
        available_range = self.get_available_range_ns(dataset="EQUS.MINI", schema=schema)
        earliest_ns = available_range[0]
        window_start_ns = int(window.start.timestamp() * 1_000_000_000)
        if earliest_ns is not None and window_start_ns >= earliest_ns:
            return None
        try:
        ...
        if fallback_frame.empty:
            return None
        tagged = fallback_frame.copy()
        tagged["source_dataset"] = fallback_dataset
        if instrument_id is not None and "instrument_id" not in tagged.columns:
