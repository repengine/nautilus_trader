...
- **Pipeline orchestrator dual path**
  The `MLPipelineOrchestrator` façade now forwards to the legacy monolith whenever `ML_USE_LEGACY_PIPELINE_ORCHESTRATOR=1`. This ensures ingestion, dataset build, HPO, and training flows keep working until the component implementation reaches feature parity. With the flag unset (or set to `0`), the façade instantiates the component stack—config, discovery, binding, ingestion, and dataset builders remain active, but training/HPO entry points still warn and return `1`. Unit coverage lives in `ml/tests/unit/orchestration/test_pipeline_orchestrator_component.py`, and the façade raises explicit `RuntimeError`s if a required component was skipped. Operators can override at runtime by exporting the environment variable before launching the CLI or service.
