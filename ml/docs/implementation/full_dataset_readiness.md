...
- [x] Regenerate `data/features/events/events.parquet` via `EventIngestionUtility` for the Tier‑1 horizon (FOMC, CPI stubs, holidays). (`PYTHONPATH=. python -m ml.preprocessing.event_ingestion` style utility run on 2025‑11‑08 now writes ~326k events spanning 2023‑2025, including ALFRED releases + options/FOMC/holiday stubs.)
...
- [x] Run `ml/cli/ingest_earnings.py` (or `make earnings-ingest`) with a real Postgres DSN and `ML_FILE_STORE_PATH` to hydrate `earnings_actuals/earnings_estimates` plus parquet mirrors. (`ML_FILE_STORE_PATH=data/earnings_file_store SEC_USER_AGENT_NAME='your name' SEC_USER_AGENT_EMAIL='you@example.com' SEC_USER_AGENT_PHONE='555-555-5555' poetry run python -m ml.cli.ingest_earnings --dsn postgresql://postgres:postgres@localhost:5432/nautilus --parquet-root data/features/earnings_raw --universe-mode tier1_full --quarters 4` on 2025‑11‑08 wrote ~280 actuals/70 estimates to `ml.earnings_*` and mirrored ticker-partitioned parquet. Added the `update_watermark` helper function and raw-writer support in `DataStoreLegacy` so dual writes/metrics succeed.)
...
- [x] Scheduler/orchestrator components already use domain services (`ml/data/ingest/orchestrator.py`, `ml/data/scheduler_facade.py`) and no longer rely on CLI wrappers, but they haven’t been executed end-to-end since the fixes.
...
- Macro, event, micro, and L2 datasets are parquet-only: ALFRED releases write to `data/features/macro/fred/vintages/<series>/release_calendar.parquet` (`ml/data/loaders/alfred_loader.py:150-285`), `EventIngestionUtility` writes `data/features/events/events.parquet` (`ml/preprocessing/event_ingestion.py:52-173`), and cache hydrators persist `data/features/{micro_minute,l2_minute}` partitions via `MicroMinuteCache` / `L2MinuteCache` (`ml/data/micro_cache.py:13-82`, `ml/data/l2_cache.py:15-122`, `ml/cli/hydrate_feature_caches.py:1-84`). None of these flows emit SQL rows, data events, or watermarks.
- Earnings ingestion is the only dual-write path; it instantiates `DataStore` with `EarningsParquetRawWriter` so every write hits Postgres plus `data/features/earnings_raw/**` (`ml/cli/ingest_earnings.py:20-74`, `ml/stores/earnings_raw_writer.py:38-139`). Macro refresh (`ml/data/common/macro_vintage.py`), event ingestion (`ml/core/integration.py:403-449`), and cache hydration (`ml/data/rehydration/cache_hydration.py:26-190`) bypass `RawIngestionWriterProtocol`, so orchestration telemetry never sees those data sets.
...
- **Earnings:** `EarningsStore` manages `ml.earnings_actuals` / `ml.earnings_estimates` tables, and `ml/cli/ingest_earnings.py` wires `EarningsParquetRawWriter` so every write also lands under `data/features/earnings_raw/**`. [ml/stores/earnings_store.py:65-140][ml/cli/ingest_earnings.py:20-96][ml/stores/earnings_raw_writer.py:1-120]
...
- Market ingestion can dual-write via `ParquetCatalogRawWriter` when `dual_write` is enabled; there is no analogous path for cache hydration or macro/event ingestion CLI flows. [ml/data/scheduler_facade.py:1364-1490][ml/data/ingest/orchestrator.py:320-390][ml/cli/hydrate_feature_caches.py:34-116]
