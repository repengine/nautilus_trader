...
- Final Teacher Train: 3–5 epochs on macro dataset, using BCE with `pos_weight=auto`
...
- Launch HPO (GPU, precision 32):
  - `PYTHONPATH=. .venv/bin/python -m ml.cli.hpo_tft \
    ...
    --out_dir /tmp/tft_universe_7y_l0_pilot/hpo_l0_macro \
    --feature_registry_dir ~/.nautilus/ml/features \
    --feature_set_id feature_set_1757890539197189 \
    --epochs 3 --workers 4 --batch_size 256 \
    --accelerator gpu --devices 1 --val_days 14 --precision 32`

- Final Teacher (example):
  - `.venv/bin/python -m ml.training.teacher.tft_cli \
    --train_data_parquet /tmp/tft_universe_7y_l0_pilot/merged_macro_fullfred/dataset.parquet \
    --out_dir /tmp/tft_universe_7y_l0_pilot/teacher_macro \
    --model_id tft_teacher_final_l0_macro \
    --feature_registry_dir ~/.nautilus/ml/features \
    --feature_set_id feature_set_1757890539197189 \
    --max_epochs 3 --loss bce --pos_weight auto \
    --dataloader_workers 4 --accelerator gpu --devices 1 \
    --batch_size 256 --val_days 14 --precision 32 \
    --static_categoricals instrument_id,asset_class,exchange \
    --known_future_reals tod_sin,tod_cos,dow_sin,dow_cos,is_market_open,is_premarket,is_aftermarket \
