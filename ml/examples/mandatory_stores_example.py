...
This example shows how all ML data is automatically persisted without
any additional configuration required from the user.
"""
...
class ProductionMLActor(BaseMLInferenceActor):
    ...
    All features, predictions, and signals are automatically stored
    to the configured database without any additional code needed.
    """
    ...
    def __init__(self, config: MLActorConfig):
        ...
        self.feature_names = [
            ...
            "volatility_20", "rsi_14", "macd_signal"
        ]
        
        print(f"Initialized ProductionMLActor with automatic stores:")
    ...
    def _compute_features(self, bar: Bar) -> np.ndarray:
        ...
        These features are AUTOMATICALLY persisted to the feature store
        by the base class. No additional code needed!
        """
    ...
    def _predict(self, features: np.ndarray) -> tuple[float, float]:
        ...
        The prediction and confidence are AUTOMATICALLY persisted to
        the model store by the base class.
        """
...
def main():
    ...
    actor = ProductionMLActor(production_config)
