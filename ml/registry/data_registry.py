...
class DataRegistry(MLComponentMixin):
    ...
    def get_pipeline_signature(self, dataset_id: str) -> str | None:
        ...
        Examples
        --------
        >>> signature = registry.get_pipeline_signature("my_dataset")
        >>> if signature:
    ...
    def set_pipeline_signature(self, dataset_id: str, signature: str) -> None:
        ...
        KeyError
            If dataset doesn't exist
        ValueError
            If signature is empty or invalid format

        Examples
        --------
        >>> from ml.orchestration.signature import compute_pipeline_signature
        >>> signature = compute_pipeline_signature(["returns_1", "rsi_14"], "1.0")
        >>> registry.set_pipeline_signature("my_dataset", signature)

        """
        if not signature:
            msg = "Pipeline signature cannot be empty"
        ...
        with self._lock:
            ...
            # Update manifest using update_manifest method
            self.update_manifest(dataset_id, {"metadata": metadata})
