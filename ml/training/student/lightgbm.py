...
class LightGBMStudentDistiller:
    def __init__(
        ...
        if opset is None:
            try:
                from ml.training.export import DEFAULT_ONNX_OPSET  # local import to avoid heavy deps at import time
    ...
    def _teacher_logits(q: npt.NDArray[np.float32], eps: float = 1e-6) -> npt.NDArray[np.float32]:
        ...
        result: npt.NDArray[np.float32] = np.log(clipped / (1.0 - clipped)).astype(np.float32)
        return result
    ...
    def fit(
        ...
        val_set = lgb.Dataset(X_val, label=val_labels)
        ...
        self.model = lgb.train(  # type: ignore[call-arg]
            params,
            train_set,
            valid_sets=[val_set],
            fobj=fobj,
    ...
    def export_onnx(
        ...
        if (
            ...
            a_name = "platt_a"
            b_name = "platt_b"
            if onnx_numpy_helper is None or onnx_helper is None:
                raise ImportError("Required ONNX helper components not available")
            a_init = onnx_numpy_helper.from_array(
                np.array(self._platt_coef, dtype=np.float32),
                name=a_name,
            )
            b_init = onnx_numpy_helper.from_array(
                np.array(self._platt_intercept, dtype=np.float32),
                name=b_name,
            )
            initializers.extend([a_init, b_init])
            mul_out = "platt_mul_out"
            add_out = "platt_add_out"
            nodes.append(onnx_helper.make_node("Mul", inputs=[last, a_name], outputs=[mul_out]))
        ...
        if onnx_mod is None:
            raise ImportError("Required ONNX module not available")
        onnx_mod.save(onnx_model, onnx_path)
