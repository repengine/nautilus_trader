...
class PersistenceComponent:
    ...
    def save_model(self, path: str | Path) -> None:
        ...
        reproducibility_provenance = self._resolve_reproducibility_provenance()
        manifest = self._create_model_manifest(
            save_path,
            reproducibility_provenance=reproducibility_provenance,
        )
    ...
    def _create_model_manifest(
        self,
        save_path: Path,
        *,
        reproducibility_provenance: dict[str, ReproducibilityValue] | None = None,
    ) -> ModelManifest:
        """
        ...
        reproducibility_provenance : dict[str, ReproducibilityValue] | None, optional
            Canonical reproducibility payload to embed in manifest training config.
        ...
        resolved_reproducibility = (
            dict(reproducibility_provenance)
            if reproducibility_provenance is not None
            else self._resolve_reproducibility_provenance()
        )
        ...
        from ml.training.export import enforce_positive_class_mapping

        decision_config = enforce_positive_class_mapping(
            self._trainer._model,
            getattr(self._trainer._config, "decision_config", {}),
        )
        training_config = self._trainer._config_to_dict()
        training_config["reproducibility"] = resolved_reproducibility
    ...
    def _resolve_reproducibility_provenance(self) -> dict[str, ReproducibilityValue]:
        """
        Resolve canonical reproducibility metadata for model persistence boundaries.
        """
        deterministic_mode_raw = getattr(self._trainer._config, "deterministic_mode", None)
        deterministic_mode = (
            bool(deterministic_mode_raw)
            if isinstance(deterministic_mode_raw, bool)
            else ActorRemediationPolicyConfig.from_env().deterministic_mode
        )
        return build_configured_reproducibility_provenance(
            primary_seed=getattr(self._trainer._config, "random_seed", None),
            deterministic_mode=deterministic_mode,
            context="training random seed",
        )
