...
from ml.data.ingest.service import DatabentoHistoricalClient
...
def _coerce_registry_path(registry_path: str | Path | None) -> Path | None:
    """
    Normalize optional registry path values from CLI/runtime call sites.

    Args:
        registry_path: Raw path value supplied by argparse or function callers.

    Returns:
        ``Path`` value when provided, otherwise ``None``.
    """
...
def _resolve_registry_path(registry_path: Path | None) -> Path:
    """
    Resolve registry path with the module default fallback.

    Args:
        registry_path: Optional explicit registry path.

    Returns:
        Effective registry path.
    """
    return registry_path if registry_path is not None else Path("ml_registry")


def _auto_persistence_config(registry_path: Path | None) -> tuple[PersistenceConfig, Path | None]:
    """
    Build persistence configuration via environment-aware auto-detection.

    Args:
        registry_path: Optional registry path used for JSON backend fallback.

    Returns:
        Tuple of resolved persistence configuration and possibly-updated
        registry path (when JSON fallback is selected).
    """
    ...
    logger.info(f"Using JSON backend at {resolved_registry_path}")
    return (
        PersistenceConfig(
            backend=BackendType.JSON,
            json_path=resolved_registry_path,
        ),
        resolved_registry_path,
    )


def _backend_override_persistence_config(
    backend_name: str | None,
    *,
    registry_path: Path | None,
    require_postgres_connection: bool,
) -> PersistenceConfig | None:
    """
    Resolve optional backend override from CLI flags.

    Args:
        backend_name: Backend token from CLI (``json`` or ``postgres``).
        registry_path: Optional registry path used for JSON backend.
        require_postgres_connection: Whether postgres override must include
            ``NAUTILUS_REGISTRY_DB_URL``.

    Returns:
        Persistence configuration for the override, or ``None`` when no override
        was requested.

    Raises:
        SystemExit: If postgres override requires an environment connection
            string and none is available.
    """
    if backend_name is None:
        return None

    backend = BackendType.JSON if backend_name == "json" else BackendType.POSTGRES
    if backend is BackendType.JSON:
        return PersistenceConfig(
            backend=backend,
            json_path=_resolve_registry_path(registry_path),
        )

    if not require_postgres_connection:
        return PersistenceConfig(backend=backend)
...
class CoverageReporter:
    ...
    def __init__(
        ...
        """
        resolved_persistence = persistence_config
        if resolved_persistence is None:
            resolved_persistence, registry_path = _auto_persistence_config(registry_path)

        self.persistence = PersistenceManager(resolved_persistence)
        self.backend = resolved_persistence.backend
...
def plan_backfill(
    ...
    if resolved_persistence.backend == BackendType.POSTGRES:
...
def apply_backfill(
    ...
    if not dry_run and api_key:
        try:
            ...
            logger.error("Failed to initialise Databento ingestion service", exc_info=True)
            print(f"\nFailed to initialise Databento ingestion service: {exc}")
            sys.exit(1)
    ...
    for i, gap_info in enumerate(gaps, 1):
        ...
        def _to_dataframe(obj: object) -> pd.DataFrame | None:
            try:
                import pandas as pd
            except Exception:
                return None
            try:
                to_df = getattr(obj, "to_df", None)
                if callable(to_df):
                    return cast(pd.DataFrame, to_df())
...
Environment Variables:
  ...
  NAUTILUS_CATALOG_PATH - Path to data catalog (default: ./catalog)
    ...
    if args.command == "report":
        reporter_registry_path = _coerce_registry_path(getattr(args, "registry_path", None))
        persistence_config = _backend_override_persistence_config(
            args.backend,
            registry_path=reporter_registry_path,
            require_postgres_connection=False,
        )
        ...
        registry_path = _coerce_registry_path(getattr(args, "registry_path", None))
        persistence_config = _backend_override_persistence_config(
            args.backend,
...
__all__ = [
    "CoverageReporter",
    "_auto_persistence_config",
    "_backend_override_persistence_config",
    "_coerce_registry_path",
