    ...
    def visit_Call(self, node):
        ...
        if self.current_function:
            ...
            if isinstance(node.func, ast.Name) and node.func.id == "assert":
                ...
                if node.func.attr in ["assertEqual", "assertTrue", "assertFalse", "assert_allclose"]:
...
def generate_report(analyzer, test_files):
    ...
    report.append(f"- Unique test targets: {len(set(functools.reduce(operator.iadd, analyzer.test_targets.values(), [])))}\n")
    ...
    if patterns["example_with_property"]:
        report.append("2. **Remove redundant example tests** - Property tests already cover these\n")

    if patterns["could_be_parameterized"]:
        report.append("3. **Use parametrized tests** - Reduce code duplication with @pytest.mark.parametrize\n")

    if overlapping:
        high_overlap = [t for t, tests in overlapping.items() if len(tests) > 3]
        if high_overlap:
            report.append(f"4. **Consolidate overlapping tests** - {len(high_overlap)} targets are tested by 4+ tests\n")
...
def main(test_dir, output, verbose):
    ...
    patterns = identify_redundant_patterns(analyzer)
    if patterns["example_with_property"]:
        print(f"Example tests redundant with property tests: {len(patterns['example_with_property'])}")
