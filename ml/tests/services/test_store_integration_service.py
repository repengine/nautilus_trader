...
def integration_manager(
    ...
    patch_engine_manager: Callable[..., ContextManager[MagicMock]],
) -> Iterator[_StubIntegrationManager]:
    """
    Provide an integration manager wired to the shared EngineManager harness.

    Args:
        store_integration_metrics_database: Seeded database for metrics aggregation tests.
        patch_engine_manager: Fixture that patches EngineManager to reuse the seeded engine.

    Yields:
        Stub integration manager with a database connection string.
    """
...
async def test_store_metrics_snapshot_aggregates_real_data(
    ...
    """Verify store metrics aggregation against seeded database state."""
