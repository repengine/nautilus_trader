...
including full training pipelines, ONNX export verification, performance benchmarks,
and integration with all advanced features (GPU, Optuna, MLflow).
...
from ml._imports import HAS_MLFLOW, HAS_OPTUNA, HAS_XGBOOST
...
class TestUnifiedXGBoostIntegration:
    ...
    def test_feature_decay_tracking_integration(self, sample_financial_data, basic_training_config):
        ...
        if len(feature_names) > 2:
            assert len(decay_summary["current_alerts"]) >= 0  # May or may not trigger depending on data
    ...
    def test_optuna_integration_mocked(self, mock_optuna, sample_financial_data, full_feature_config):
    ...
    def test_mlflow_integration_mocked(self, mock_mlflow, sample_financial_data, full_feature_config):
...
class TestUnifiedXGBoostPerformanceRequirements:
    ...
    def test_inference_latency_requirement(self, sample_financial_data):
        ...
        assert p99_latency < 5.0, f"P99 inference latency {p99_latency:.2f}ms exceeds 5ms requirement"
