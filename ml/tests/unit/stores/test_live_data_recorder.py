...
from ml.tests.utils.wait_helpers import async_wait_for_coroutine_condition
...
async def test_recorder_memory_usage_remains_stable(
    ...
    for cycle in range(10):
        ...
        # Wait for flush using event-based approach
        await async_wait_for_condition(
            lambda: recorder.data_registry.emit_event.call_count > cycle,
            timeout=1.0,
            poll_interval=0.01,
            error_message=f"Flush {cycle} did not occur"
        )
...
async def test_recorder_periodic_flush(
    ...
    # Wait for periodic flush using event-based approach
    await async_wait_for_condition(
        lambda: recorder.data_registry.emit_event.call_count > 0,
        timeout=1.0,
        poll_interval=0.02,
        error_message="Periodic flush did not trigger"
    )
