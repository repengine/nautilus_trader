...
def test_aggregate_metrics_by_window_preserves_totals() -> None:
    rows = [
        {"metric_name": "ml_predictions_total", "domain": "models", "instrument_id": "EURUSD", "value": 1.0, "timestamp": 1000},
        {"metric_name": "ml_predictions_total", "domain": "models", "instrument_id": "EURUSD", "value": 2.0, "timestamp": 1001},
    ]
...
def test_scale_health_scores_clips_and_scales() -> None:
    ...
    assert float(out.loc[0, "health_score"]) <= 1.0
    assert float(out.loc[1, "health_score"]) > 0.6
