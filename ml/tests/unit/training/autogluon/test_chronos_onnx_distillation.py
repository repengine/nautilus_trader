"""
Unit tests for Chronos -> ONNX distillation artifacts.
"""
...
def _register_features(tmp_path: Path, feature_names: list[str]) -> tuple[str, str]:
    ...
    return str(registry_dir), feature_set_id
...
def test_prepare_chronos_onnx_distillation_artifacts_alignment(
    tmp_path: Path,
    ...
    registry_dir, feature_set_id = _register_features(tmp_path, feature_names)
    ...
    assert teacher_npz["q_train"].shape == (2,)
    assert teacher_npz["q_val"].shape == (2,)
    assert np.allclose(teacher_npz["q_train"], 0.0)

    features_npz = np.load(artifacts.features_npz_path, allow_pickle=True)
    assert features_npz["X_train"].shape == (2, 2)
    assert features_npz["X_val"].shape == (2, 2)
    assert list(features_npz["feature_names"].tolist()) == feature_names


@pytest.mark.skipif(not HAS_POLARS, reason="Polars not available")
def test_prepare_chronos_onnx_distillation_artifacts_sigmoid(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from ml.training.autogluon import soft_label_generator as slg
    ...
    teacher_npz = np.load(artifacts.teacher_preds_path, allow_pickle=True)
    assert np.allclose(teacher_npz["q_train"], 0.5)
