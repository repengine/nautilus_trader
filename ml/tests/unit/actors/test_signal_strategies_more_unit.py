...
def test_extremes_strategy_detects_top_extreme() -> None:
    ...
    sig = strat.generate_signal(
        bar=_stub_bar(), prediction=0.95, confidence=0.9, features=np.zeros(1, dtype=np.float32), context=ctx
    )
...
def test_momentum_strategy_requires_slope() -> None:
    ...
    sig = strat.generate_signal(
        bar=_stub_bar(), prediction=0.3, confidence=0.9, features=np.zeros(1, dtype=np.float32), context=ctx
    )
    assert isinstance(sig, MLSignal)
