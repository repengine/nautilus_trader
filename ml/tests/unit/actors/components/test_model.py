...
@pytest.mark.unit
def test_model_reject_pickle_files(pickle_model_file: Path) -> None:
    """
    ...
    """
    config = create_config(str(pickle_model_file))
    component = ModelComponent(config=config, logger=logging.getLogger("test"))
    with pytest.raises(ValueError, match="Pickle files not allowed.*use ONNX"):
        component.load_model()
...
def test_model_onnx_validation(tmp_path: Path) -> None:
    ...
    with pytest.raises((ValueError, RuntimeError), match="Invalid ONNX model|Failed to load ONNX"):
        component.load_model()
...
def test_model_warm_up_preallocates_buffers(
    ...
    assert component._warmup_iterations == 10
