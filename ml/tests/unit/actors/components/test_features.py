...
Implementation: ml/actors/components/features.py
...
def test_warm_up_buffer_prefill(
    ...
    # Then
    assert len(component.get_buffered_bars()) == 30, f"Expected buffer size 30, got {len(component.get_buffered_bars())}"
    assert component.is_warmed_up() is True, "Component should be warmed up after pre-filling with 30 bars"
