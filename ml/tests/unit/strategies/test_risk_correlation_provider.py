...
class BrokenCorrelationProvider:
    def get_correlation_snapshot(
        self,
        inst1: InstrumentId,
        inst2: InstrumentId,
...
def test_risk_uses_fallback_when_correlation_provider_missing(
    isolated_prometheus_registry: object,
) -> None:
    inst_a = InstrumentId.from_str("AAA.SIM")
    inst_b = InstrumentId.from_str("BBB.SIM")
    ...
    metric_value = isolated_prometheus_registry.registry.get_sample_value(
        "ml_risk_correlation_degraded_total",
        labels={"reason": "provider_missing"},
    )
    assert metric_value == 1.0


def test_risk_uses_fallback_when_correlation_provider_errors(
    isolated_prometheus_registry: object,
) -> None:
    inst_a = InstrumentId.from_str("AAA.SIM")
    inst_b = InstrumentId.from_str("BBB.SIM")
    ...
    manager = RiskManager(
        ...
        positions_provider=positions_provider,
        correlation_provider=BrokenCorrelationProvider(),
    )

    assert manager._check_correlation_limits(inst_a, DummyPortfolio()) is True
    metric_value = isolated_prometheus_registry.registry.get_sample_value(
        "ml_risk_correlation_degraded_total",
        labels={"reason": "provider_error"},
    )
    assert metric_value == 1.0


def test_risk_uses_fallback_when_correlation_value_invalid(
    isolated_prometheus_registry: object,
) -> None:
    inst_a = InstrumentId.from_str("AAA.SIM")
    inst_b = InstrumentId.from_str("BBB.SIM")
    positions_provider = DummyPositionsProvider([DummyPosition(inst_b)])
    provider = DummyCorrelationProvider(
        CorrelationSnapshot(
            value=1.5,
            ts_event=time.time_ns(),
            source="provider",
