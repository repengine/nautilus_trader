...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--catalog_clean_mode",
        default=os.getenv("CATALOG_CLEAN_MODE", ""),
        help="Optional catalog hygiene strategy (e.g. archive)",
    )
    parser.add_argument(
        "--catalog_backup_dir",
        default=os.getenv("CATALOG_BACKUP_DIR", ""),
        help="Optional directory for catalog archives (defaults to catalog parent)",
    )
...
def _execute_with_namespace(
    ...
    *,
    ...
    raw_writer: RawIngestionWriterProtocol | None = None
    catalog_clean_mode = (args.catalog_clean_mode or "").strip().lower()
    catalog_backup_dir = args.catalog_backup_dir.strip() if args.catalog_backup_dir else ""
    if args.catalog_path:
        catalog_path_obj = Path(args.catalog_path)
        if catalog_clean_mode:
            if catalog_clean_mode == "archive":
                backup_root = Path(catalog_backup_dir).expanduser() if catalog_backup_dir else None
                ...
                logger.warning(
                    ...
                    extra={
                        "mode": args.catalog_clean_mode,
                        "catalog": str(catalog_path_obj),
                    },
                )
        try:
            from nautilus_trader.persistence.catalog.parquet import ParquetDataCatalog

            parquet_catalog = ParquetDataCatalog(str(catalog_path_obj))
