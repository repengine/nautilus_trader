"""
ML pipeline health monitoring services.
...
def run_pipeline_health_checks(
    connection_string: str,
) -> tuple[dict[str, ComponentHealth], HealthStatus, int]:
    """
    Execute all pipeline health checks for a database connection.

    Args:
        connection_string: PostgreSQL connection string.

    Returns:
        Tuple containing component health map, overall status, and exit code.
    """
    with PipelineHealthChecker(connection_string) as checker:
        component_health = checker.check_all_components()
        overall_status, exit_code = checker.get_overall_status(component_health)
    return component_health, overall_status, exit_code
