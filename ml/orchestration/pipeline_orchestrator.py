...
class MLPipelineOrchestrator:
    ...
    def run_hpo(self, cfg: HPOConfig | None, dataset_csv: Path, out_dir: Path) -> int:
        if cfg is None or not cfg.enabled or self.hpo_main is None:
            return 0
    ...
    def distill_student(
        ...
        *,
        ...
        if parent_model_id is None:
            logger.error("parent_model_id is required for student registration (provide cfg.parent_model_id or teacher_cfg)")
            return 1
    ...
    def run(
        ...
        *,
        ...
        Args:
            ...
            checkpoint_file: Optional path to checkpoint file for saving/loading state
            resume: If True and checkpoint_file exists, resume from saved checkpoint
        ...
        Example:
            ...
            >>> # Single-symbol run
            >>> config = OrchestratorConfig(dataset=DatasetBuildConfig(symbols="AAPL", ...))
            >>> orchestrator.run(config)
            >>> # Multi-symbol run with result isolation
            >>> config = OrchestratorConfig(dataset=DatasetBuildConfig(symbols="AAPL,GOOGL,MSFT", ...))
            >>> orchestrator.run(config)
            >>> # Resume from checkpoint after interruption
            >>> orchestrator.run(config, checkpoint_file=Path("/tmp/checkpoint.json"), resume=True)
        ...
        symbols = self._parse_symbols(cfg.dataset.symbols)

        if len(symbols) == 1:
            # Single symbol - use existing behavior
    ...
    def _parse_symbols(self, symbols_str: str) -> list[str]:
        ...
        Args:
            symbols_str: Comma-separated symbol string (e.g., "AAPL,GOOGL,MSFT")

        Returns:
            List of trimmed symbol strings

        Example:
            >>> orchestrator._parse_symbols("AAPL")
            ['AAPL']
            >>> orchestrator._parse_symbols("AAPL,GOOGL,MSFT")
            ['AAPL', 'GOOGL', 'MSFT']
    ...
    def _create_symbol_config(
        ...
        When:
        - Creating symbol-specific configuration

        Then:
        - Output directory includes symbol subdirectory
        - Symbol field contains only the single symbol
        - All other config fields preserved

        Args:
            cfg: Base orchestrator configuration
            symbol: Single symbol to process

        Returns:
            Isolated OrchestratorConfig for the symbol

        Example:
            >>> base_cfg = OrchestratorConfig(dataset=DatasetBuildConfig(
            ...     symbols="AAPL,GOOGL",
            ...     out_dir="/tmp/output",
            ... ))
            >>> aapl_cfg = orchestrator._create_symbol_config(base_cfg, "AAPL")
            >>> aapl_cfg.dataset.symbols
            'AAPL'
            >>> aapl_cfg.dataset.out_dir
            '/tmp/output/AAPL'
    ...
    def _run_multi_symbol(
        ...
        *,
        ...
        Given:
        - Multiple symbols to process
        - Same pipeline configuration for all symbols

        When:
        - Processing each symbol independently

        Then:
        - Each symbol gets isolated output directory
        - No cross-contamination between symbol datasets
        - All symbols processed successfully
        - Return 0 if all succeed, non-zero if any fail

        Args:
            cfg: Base orchestrator configuration
            symbols: List of symbols to process
            checkpoint_file: Optional path to checkpoint file
            resume: If True and checkpoint_file exists, resume from checkpoint

        Returns:
            Exit code (0 if all symbols succeed, non-zero if any fail)

        Example:
            >>> orchestrator._run_multi_symbol(
            ...     cfg,
            ...     ["AAPL", "GOOGL", "MSFT"],
        ...
        for symbol in symbols:
            ...
            # Note: checkpoint_file is shared across symbols, but each symbol
            # has its own output directory, so state is isolated
        ...
        # Return 0 if all symbols succeeded, else return 1
    ...
    def _run_single_symbol(
        ...
        *,
        ...
        This contains the original run() implementation for single-symbol processing.

        Args:
            cfg: Orchestrator configuration (must have single symbol)
            checkpoint_file: Optional path to checkpoint file for saving/loading state
            resume: If True and checkpoint_file exists, resume from saved checkpoint
