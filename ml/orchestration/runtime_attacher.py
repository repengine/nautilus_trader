...
class RuntimeAttacher:
    ...
    def _attach_runtime(
        ...
        if manager is not None:
            attach_stores = getattr(manager, "attach_stores", None)
            attach_regs = getattr(manager, "attach_registries", None)
            if callable(attach_stores):
                try:
                    attach_stores(
                    ...
                    logger.debug("attach_stores stub error", exc_info=True)
            if callable(attach_regs):
                try:
                    attach_regs(
                    ...
                    logger.debug("attach_registries stub error", exc_info=True)
    ...
    def run(self) -> None:
        """
        Structural placeholder run; attaches runtime and runs validators.
        """
