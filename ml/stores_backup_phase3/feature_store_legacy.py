...
class FeatureStoreLegacy(HealthMixin, BusPublisherMixin, DataRegistryMixin):
    """
    LEGACY: Unified feature computation and storage for ML pipeline.

    This is the original god class implementation preserved for backward compatibility
    and comparison testing. Use FeatureStore (facade) for new code.
    ...
    @with_fallback(
        fallback_value=None, log_level="warning", operation_name="emit feature computation event"
    )
    ...
    def get_latest_at_or_before(self, instrument_id: str, ts_event: int) -> dict[str, float] | None:
        ...
        ts_norm = sanitize_timestamp_ns(
            int(ts_event), context="feature_store.get_latest_at_or_before"
        )
        feature_set_id = self._get_feature_set_id()
        query = (
