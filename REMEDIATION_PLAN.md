...
- [x] **Feature parity & performance guardrails**: Align FeatureCalculator to return buffer views, include microstructure/trade_flow online approximations, remove per-call allocations, and fix legacy facade enum coercion; validated with perf/parity shards plus strict `mypy`/`ruff` [AGENTS.md].
- [x] **TFT dataset builder/pipeline correctness**: Restore legacy-private shims (`_dataset_serializer`, `_validation_splitter`), retain raw `close` in outputs, and gate parquet fallback behind `ML_TFT_ALLOW_PARQUET_FALLBACK`; validated with TFT dataset builder E2E suite plus strict `mypy`/`ruff`.
- [x] **Cross-asset service wiring**: Cross-asset FeatureStore service integration shard is green; no additional wiring changes required.
- [x] **Pipeline orchestrator & dashboard parity**: Restore legacy positional parity for distillation, add defensive delegation guards in cold paths, normalize dataset metadata vintage_policy parsing, and flatten streaming result payloads (telemetry + calibration summary) with `/api/training/streaming/state` endpoint; validated with orchestrator unit/parity shards and dashboard streaming monitor tests.
